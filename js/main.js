...
function getDisplayName(place) {
  ...
  async function readExcel(filePath) {
    // Performance: pakai cache browser agar Excel tidak diunduh ulang setiap refresh.
...
function normalizeFasilitasKesehatan(row, index) {
      ...
      nearestRankList.querySelectorAll("[data-rank-place]").forEach(item => {
        item.addEventListener("click", () => {
          ...
          if (place) {
            ...
            if (map) map.setView([place.lat, place.lng], 17, { animate: false });
          }
        });
      });
    }
  ...
  function showInsightDashboard() {
    if (insightDashboard) {
      ...
      // Performance: animasi dashboard tidak di-restart setiap render.
  ...
  function focusNearestPlace() {
    ...
    if (map) {
      map.setView([nearest.lat, nearest.lng], 17, { animate: false });
    }
  ...
  try {
    ...
    console.info(`Total data valid: ${places.length}`);
  ...
  function renderMarkers() {
    ...
    visiblePlaces.forEach(place => {
      const marker = L.marker([place.lat, place.lng], {
        ...
        // Popup dibuat saat marker diklik, bukan saat semua marker dirender.
        e.target.bindPopup(buildMarkerPopup(place, status), {
          maxWidth: 320,
          minWidth: 250,
          closeButton: true,
    ...
    if (emergencyCountEl) {
      emergencyCountEl.textContent = places.filter(p => p.category === "emergency").length;
    }
...
function handleResponsivePanels() {
  ...
  async function initMap() {
    ...
    Object.keys(categories).forEach(key => {
      layers[key] = (typeof L.markerClusterGroup === "function")
  ...
  toggleTopPanel.addEventListener("click", () => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (savedTheme === "dark" || savedTheme === "light") {
      ...
      applyBasemapClass(basemapSelect.value || "dark");
    }
